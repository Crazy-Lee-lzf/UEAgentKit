...
private:
	...
	bool TryPrepareAssetForDiskRollbackResult(const FString& AssetPath, TSharedPtr<FJsonObject>& OutResult, FString& OutErrorCode, FString& OutErrorMessage) const;
	...
	bool TryDiagnoseAdditiveAnimationResult(
		...
		FString& OutErrorCode,
		FString& OutErrorMessage) const;
	bool TryEvaluateAnimationWithBasePoseResult(
		const TArray<FString>& AnimationPaths,
		const TArray<FString>& BoneNames,
		bool bLoadIfNeeded,
		TSharedPtr<FJsonObject>& OutResult,
