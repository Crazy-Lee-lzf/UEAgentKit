...
namespace UEAgentKitEditorBridgePrivate
{
	...
	const TCHAR* const PluginVersion = TEXT("0.7.0");
	const TCHAR* const DevelopmentLine = TEXT("0.7.0");
...
void FUEAgentKitEditorBridge::ProcessLine(FClientConnection& Client, const TArray<uint8>& LineBytes)
{
	...
	else if (Method == TEXT("editor.diagnoseAdditiveAnimation"))
	{
		const TArray<TSharedPtr<FJsonValue>>* AnimationPaths = nullptr;
		Params->TryGetArrayField(TEXT("animationPaths"), AnimationPaths);
		bool bLoadIfNeeded = false;
		...
		FString ErrorMessage;
		SendActionResult(
			TryDiagnoseAdditiveAnimationResult(
				ParsedAnimationPaths, bLoadIfNeeded, Result, ErrorCode, ErrorMessage),
			Result,
			ErrorCode,
			ErrorMessage);
	}
	else if (Method == TEXT("editor.evaluateAnimationWithBasePose"))
	{
		const TArray<TSharedPtr<FJsonValue>>* AnimationPaths = nullptr;
		const TArray<TSharedPtr<FJsonValue>>* BoneNames = nullptr;
		Params->TryGetArrayField(TEXT("animationPaths"), AnimationPaths);
		...
		FString ErrorMessage;
		SendActionResult(
			TryEvaluateAnimationWithBasePoseResult(
				ParsedAnimationPaths, ParsedBoneNames, bLoadIfNeeded, Result, ErrorCode, ErrorMessage),
			Result,
			ErrorCode,
			ErrorMessage);
