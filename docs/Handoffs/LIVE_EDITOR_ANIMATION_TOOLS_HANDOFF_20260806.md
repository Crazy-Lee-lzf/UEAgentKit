...
- C++ 动画 Live Write Operation；
- Operation Registry 接入；
- Animation Data Asset Reader 持久化字段；
- Python Patch 专用值校验；
- MCP `ue_plan_animation_scale_fix`；
- Tool Registry 更新；
- 保存后动画字段独立验证；
- 单资产 Live Apply / Undo Smoke；
- Authorized Save Smoke；
- 文档和 Policy 示例。

关键未跟踪文件：

```text
Plugin/UEAgentKit/Source/UEAgentKitEditor/Private/LiveWriteAnimationOperations.cpp
docs/ANIMATION_SCALE_FIX_TOOL.md
tests/integration/mcp_live_animation_scale_fix_smoke.py
tests/integration/mcp_save_animation_scale_fix.py
```
...
还没有批量 Audit、批量 Plan、Change Set 批量 Apply、批量 Save 和 Rollback。
