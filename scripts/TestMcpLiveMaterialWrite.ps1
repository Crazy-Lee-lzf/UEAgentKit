...
$FixturePlan = Join-Path $ToolRoot "tests\fixtures\material_live_write_plan.json"
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_material_write_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\McpLiveMaterialWriteSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpLiveMaterialWriteSmoke"
...
$MaterialInstanceClass = "/Script/Engine.MaterialInstanceConstant"
$EditorProcess = $null
$FixtureHashes = @{}
$DatabaseHashBefore = ""
...
try
{
    ...
    Write-Host "Resetting isolated material fixtures..."
    & $FixtureScript `
    ...
    {
        throw "Material fixture reset failed with exit code $LASTEXITCODE"
    }

    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
        -Root "/Game/UEAgentKitWriteTests/Materials" `
        -Output $RevisionExport
    if ($LASTEXITCODE -ne 0)
    {
        throw "Material Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live material write smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedAssetRoots = @("/Game/UEAgentKitWriteTests/Materials")
        ...
        allowedAssetClasses = @($MaterialInstanceClass)
        allowedAssetProperties = @()
        allowedMaterialParameters = @(
            "$MaterialInstanceClass#Scalar#EmissiveIntensity",
            "$MaterialInstanceClass#Vector#TintColor",
            "$MaterialInstanceClass#Texture#BaseTexture",
            "$MaterialInstanceClass#StaticSwitch#EnableDetail"
        )
    ...
    foreach ($FixtureEntry in $Fixture.fixtures)
    {
        ...
        Assert-UeakPath -Path $PackageFile -Description "material fixture package $($FixtureEntry.id)" -PathType File
    ...
    {
        throw "MCP Live Editor material write smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
{
    ...
    if ($FixtureHashes.Count -gt 0)
    {
        ...
        {
            $Succeeded = $false
            Write-Warning "Live material write changed a fixture package on disk; resetting the fixtures."
            & $FixtureScript `
                -EngineRoot $EngineRoot `
                -ProjectPath $ProjectPath `
                -Plan $FixturePlan `
...
if (!$Succeeded)
{
    throw "MCP Live Editor material write smoke test did not complete successfully."
}
Write-Host "MCP Live Editor material write smoke test passed."
