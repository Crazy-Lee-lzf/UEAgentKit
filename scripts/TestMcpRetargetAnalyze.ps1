...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_retarget_analyze_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\McpRetargetAnalyzeSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpRetargetAnalyzeSmoke"
...
try
{
    ...
    Write-Host "Preparing retarget analyze fixtures..."
    ...
    {
        throw "Retarget Analyze Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live Retarget Analyze smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    {
        throw "MCP Live Editor Retarget Analyze smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
{
    ...
    if ($FixtureHashes.Count -gt 0)
    {
        ...
        if ($ChangedIds.Count -gt 0)
        {
            ...
            {
                $Succeeded = $false
                Write-Warning "Live Retarget Analyze changed unexpected fixture packages on disk: $($Unexpected -join ', ')"
            }
            else
            {
...
if (!$Succeeded)
{
    throw "MCP Live Editor Retarget Analyze smoke test did not complete successfully."
}
Write-Host "MCP Live Editor Retarget Analyze smoke test passed."
