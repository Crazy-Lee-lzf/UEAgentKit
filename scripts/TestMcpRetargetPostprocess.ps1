...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_retarget_postprocess_smoke.py"
...
$RetargeterFile = Join-Path $ProjectDirectory "Content\Characters\Mannequins\Meshes\IKRetargeter_SKM_Manny_Simple_to_SK_XinYueHu.uasset"
foreach ($Required in @($VenvPython, $CatalogScript, $ClientScript, $UnrealEditor, $CompiledPlugin, $RetargeterFile))
{
    Assert-UeakPath -Path $Required -Description ([System.IO.Path]::GetFileName($Required)) -PathType File
}

$Output = Join-Path $ToolRoot "Output\McpRetargetPostprocessSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpRetargetPostprocessSmoke"
...
try
{
    ...
    foreach ($Asset in @(
        ...
        "/Game/Characters/XinYueHu/Mesh/SK_XinYueHu"))
    {
        & $CatalogScript `
        ...
        {
            throw "Retarget post-process Revision Export failed for $Asset with exit code $LASTEXITCODE"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Retarget post-process smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Retarget Post-process smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Retarget Post-process smoke test did not complete successfully."
}
Write-Host "MCP Retarget Post-process smoke test passed."
