...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_additive_base_fix_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\ABF"
...
$BackupRoot = Join-Path $ToolRoot "Backups\ABF"
...
try
{
    ...
    & $CatalogScript `
        ...
        -Asset "/Game/Characters/XinYueHu/Animations/Retargeted/MM_Rifle_Jump_RecoveryAdditive_XinYueHu" `
        -Output $RevisionExport
    if ($LASTEXITCODE -ne 0)
    {
        throw "Additive base fix Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Additive base fix smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedAssetRoots = @("/Game/Characters")
        allowedReferenceRoots = @("/Game/Characters")
        allowedReferenceClasses = @()
        allowedOperations = @("setAdditiveBasePoseFix")
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Additive Base Fix smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Additive Base Fix smoke test did not complete successfully."
}
Write-Host "MCP Additive Base Fix smoke test passed."
