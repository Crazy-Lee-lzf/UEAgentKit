...
$FixturePlan = Join-Path $ToolRoot "tests\fixtures\data_table_live_write_plan.json"
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_data_table_write_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\McpLiveDataTableWriteSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpLiveDataTableWriteSmoke"
...
try
{
    ...
    Write-Host "Resetting isolated DataTable fixtures..."
    & $FixtureScript `
    ...
    {
        throw "DataTable fixture reset failed with exit code $LASTEXITCODE"
    }

    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
        -Root "/Game/UEAgentKitWriteTests/DataTables" `
        -Output $RevisionExport
    if ($LASTEXITCODE -ne 0)
    {
        throw "DataTable Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live DataTable write smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedAssetRoots = @("/Game/UEAgentKitWriteTests/DataTables")
        ...
        allowedAssetClasses = @($DataTableClass)
    ...
    foreach ($FixtureEntry in $Fixture.fixtures)
    {
        ...
        Assert-UeakPath -Path $PackageFile -Description "data table fixture package $($FixtureEntry.id)" -PathType File
    ...
    {
        throw "MCP Live Editor DataTable write smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
{
    ...
    if ($FixtureHashes.Count -gt 0)
    {
        ...
        {
            $Succeeded = $false
            Write-Warning "Live DataTable write changed a fixture package on disk; resetting the fixtures."
            & $FixtureScript `
                -EngineRoot $EngineRoot `
                -ProjectPath $ProjectPath `
                -Plan $FixturePlan `
...
if (!$Succeeded)
{
    throw "MCP Live Editor DataTable write smoke test did not complete successfully."
}
Write-Host "MCP Live Editor DataTable write smoke test passed."
