...
$FixturePlan = Join-Path $ToolRoot "tests\fixtures\closed_loop_live_write_plan.json"
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_closed_loop_smoke.py"
...
try
{
    ...
    Write-Host "Resetting isolated Closed Loop fixtures..."
    & $FixtureScript `
    ...
    {
        throw "Closed Loop fixture reset failed with exit code $LASTEXITCODE"
    }

    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
        -Root "/Game/UEAgentKitWriteTests/ClosedLoop" `
        -Output $RevisionExport
    if ($LASTEXITCODE -ne 0)
    {
        throw "Closed Loop Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live Closed Loop smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedAssetRoots = @("/Game/UEAgentKitWriteTests/ClosedLoop")
        allowedReferenceRoots = @("/Game/UEAgentKitWriteTests/ClosedLoop")
    ...
    {
        throw "MCP Live Editor Closed Loop smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
{
    ...
    if ($FixtureHashes.Count -gt 0)
    {
        ...
        {
            $Unexpected = @($ChangedIds | Where-Object { $_ -notin $ExpectedSavedFixtureIds })
            if ($Unexpected.Count -gt 0)
            {
                $Succeeded = $false
                Write-Warning "Live Closed Loop changed unexpected fixture packages on disk: $($Unexpected -join ', ')"
            }
            else
            {
                Write-Host "The authorized-saved fixtures changed on disk as expected; resetting the fixtures."
            }
            & $FixtureScript `
                -EngineRoot $EngineRoot `
                -ProjectPath $ProjectPath `
...
if (!$Succeeded)
{
    throw "MCP Live Editor Closed Loop smoke test did not complete successfully."
}
Write-Host "MCP Live Editor Closed Loop smoke test passed."
