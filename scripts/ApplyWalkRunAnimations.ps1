...
# Drives plan -> apply -> batch -> authorized save for the basic Mannequin
# locomotion animations retargeted onto XinYueHu, then LEAVES THE EDITOR OPEN so
# the saved animations can be used immediately in the project.
...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_walkrun_save_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\WalkRunAnimationSave"
...
$BackupRoot = Join-Path $ToolRoot "Backups\WalkRunAnimationSave"
...
if ($null -ne $ExistingDescriptor)
{
    ...
    {
        throw "A Live Editor Bridge is already active for this test project. Use the running Editor or close it first."
    }
...
& $CatalogScript -EngineRoot $EngineRoot -ProjectPath $ProjectPath -Asset "/Game/Characters/Mannequins/Meshes/SKM_Manny_Simple" -Output $RevisionExport
if ($LASTEXITCODE -ne 0)
{
    throw "Revision Export (source) failed with exit code $LASTEXITCODE"
}
& $CatalogScript -EngineRoot $EngineRoot -ProjectPath $ProjectPath -Asset "/Game/Characters/XinYueHu/Mesh/SK_XinYueHu" -Output $RevisionExport
if ($LASTEXITCODE -ne 0)
{
    throw "Revision Export (target) failed with exit code $LASTEXITCODE"
}
...
if ($LASTEXITCODE -ne 0)
{
    throw "Index build failed with exit code $LASTEXITCODE"
}
...
{
    throw "Walk/Run animation save smoke test failed with exit code $ClientExitCode"
}

Write-Host ""
Write-Host "======================================================"
Write-Host "SUCCESS: Basic locomotion animations saved to disk."
Write-Host "The Unreal Editor was LEFT RUNNING (PID $($EditorProcess.Id))."
Write-Host "Output assets live under /Game/UEAgentKitRetargetTests/Retargeted/RTG_*"
Write-Host "======================================================"
