...
$FixturePlan = Join-Path $ToolRoot "tests\fixtures\undo_discard_live_write_plan.json"
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_undo_discard_smoke.py"
...
$StructuredClass = "/Script/UEAgentKitEditor.UEAgentKitStructuredWriteFixtureAsset"
$MaterialClass = "/Script/Engine.MaterialInstanceConstant"
$DataTableClass = "/Script/Engine.DataTable"
$DataTableRowStruct = "/Script/UEAgentKitEditor.UEAgentKitDataTableFixtureRow"
# The smoke test saves exactly this fixture through the authorized save flow to
# prove the package-saved rejection; its disk hash is expected to change.
$ExpectedSavedFixtureId = "undo-reference-asset"
$EditorProcess = $null
$FixtureHashes = @{}
$DatabaseHashBefore = ""
...
try
{
    ...
    Write-Host "Resetting isolated Undo/Discard fixtures..."
    & $FixtureScript `
    ...
    {
        throw "Undo/Discard fixture reset failed with exit code $LASTEXITCODE"
    }

    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
        -Root "/Game/UEAgentKitWriteTests/UndoDiscard" `
        -Output $RevisionExport
    if ($LASTEXITCODE -ne 0)
    {
        throw "Undo/Discard Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live Undo/Discard smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedAssetRoots = @("/Game/UEAgentKitWriteTests/UndoDiscard")
        allowedReferenceRoots = @("/Game/UEAgentKitWriteTests/UndoDiscard")
        ...
        allowedAssetProperties = @(
            "$ScalarClass#IntValue",
            "$ReferenceClass#ObjectValue",
            "$ReferenceClass#SoftObjectValue",
            "$StructuredClass#StructValue"
        )
        allowedMaterialParameters = @(
            "$MaterialClass#Scalar#EmissiveIntensity"
        )
        allowedDataTableFields = @(
            "$DataTableClass#$DataTableRowStruct#Count",
            "$DataTableClass#$DataTableRowStruct#Label",
            "$DataTableClass#$DataTableRowStruct#bEnabled"
        )
    ...
    foreach ($FixtureEntry in $Fixture.fixtures)
    {
        ...
        Assert-UeakPath -Path $PackageFile -Description "undo fixture package $($FixtureEntry.id)" -PathType File
    ...
    {
        throw "MCP Live Editor Undo/Discard smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
    ...
    if ($FixtureHashes.Count -gt 0)
    {
        ...
        foreach ($FixtureEntry in $Fixture.fixtures)
        {
            ...
            if (Test-Path -LiteralPath $PackageFile)
            {
                ...
                if ($HashAfter -ne $FixtureHashes[$Id])
                {
                    $ChangedIds += $Id
                }
            }
        ...
        {
            $OnlyExpectedSavedFixture = (
                $ChangedIds.Count -eq 1 -and $ChangedIds[0] -eq $ExpectedSavedFixtureId)
            if (!$OnlyExpectedSavedFixture)
            {
                $Succeeded = $false
                Write-Warning "Live Undo/Discard changed unexpected fixture packages on disk: $($ChangedIds -join ', ')"
            }
            else
            {
                Write-Host "The saved-package rejection fixture changed on disk as expected; resetting the fixtures."
            }
            & $FixtureScript `
                -EngineRoot $EngineRoot `
                -ProjectPath $ProjectPath `
...
if (!$Succeeded)
{
    throw "MCP Live Editor Undo/Discard smoke test did not complete successfully."
}
Write-Host "MCP Live Editor Undo/Discard smoke test passed."
