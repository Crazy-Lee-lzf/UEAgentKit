...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_retarget_setup_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\McpRetargetSetupSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpRetargetSetupSmoke"
...
try
{
    ...
    Write-Host "Preparing retarget setup fixtures..."
    ...
    {
        throw "Retarget Setup Revision Export (source) failed with exit code $LASTEXITCODE"
    }
    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Retarget Setup Revision Export (target) failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Live Retarget Setup smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        retargetCapabilities = @("retarget.inspect", "retarget.plan", "retarget.configure")
    ...
    {
        throw "MCP Live Editor Retarget Setup smoke test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Live Editor Retarget Setup smoke test did not complete successfully."
}
Write-Host "MCP Live Editor Retarget Setup smoke test passed."
