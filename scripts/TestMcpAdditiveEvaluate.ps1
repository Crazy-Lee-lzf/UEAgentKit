...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_evaluate_additive.py"
...
$Output = Join-Path $ToolRoot "Output\McpAdditiveEvaluateSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpAdditiveEvaluateSmoke"
...
try
{
        ...
        "/Game/Characters/XinYueHu/Mesh/SK_XinYueHu"))
    {
        ...
        {
            throw "Additive evaluation Revision Export failed for $Asset with exit code $LASTEXITCODE"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Additive evaluation smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Additive Evaluation smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Additive Evaluation smoke test did not complete successfully."
}
Write-Host "MCP Additive Evaluation smoke test passed."
