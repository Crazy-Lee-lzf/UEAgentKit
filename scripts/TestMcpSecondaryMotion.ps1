...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_secondary_motion_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\McpSecondaryMotionSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpSecondaryMotionSmoke"
...
try
{
    ...
    {
        throw "Skeletal secondary-motion Revision Export failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Skeletal secondary-motion smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Skeletal Secondary-Motion smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Skeletal Secondary-Motion smoke test did not complete successfully."
}
Write-Host "MCP Skeletal Secondary-Motion smoke test passed."
