...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_retarget_closed_loop.py"
...
$Output = Join-Path $ToolRoot "Output\McpAnimationRetargetClosedLoop"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpAnimationRetargetClosedLoop"
...
$ClosedLoopContent = Join-Path $ProjectDirectory "Content\UEAgentKitRetargetTests\ClosedLoop"
$EditorProcess = $null
$Succeeded = $false
...
try
{
    ...
    if (Test-Path -LiteralPath $ClosedLoopContent)
    {
        Remove-Item -LiteralPath $ClosedLoopContent -Recurse -Force
    }
    ...
    {
        throw "Retarget Closed-Loop Revision Export (source) failed with exit code $LASTEXITCODE"
    }
    & $CatalogScript `
        -EngineRoot $EngineRoot `
        -ProjectPath $ProjectPath `
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Retarget Closed-Loop Revision Export (target) failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Retarget Closed-Loop smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        retargetCapabilities = @("retarget.inspect", "retarget.plan", "retarget.configure", "retarget.batch", "retarget.validate")
    ...
    {
        throw "MCP Live Editor Animation Retarget closed-loop test failed with exit code $ClientExitCode"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Live Editor Animation Retarget closed-loop test did not complete successfully."
}
Write-Host "MCP Live Editor Animation Retarget closed-loop test passed."
