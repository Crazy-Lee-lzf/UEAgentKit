...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_additive_scale_fix_smoke.py"
...
$Output = Join-Path $ToolRoot "Output\ASF"
...
$BackupRoot = Join-Path $ToolRoot "Backups\ASF"
...
try
{
        ...
        "/Game/Characters/XinYueHu/Animations/Retargeted/MM_Rifle_Jump_RecoveryAdditive_XinYueHu"))
    {
        ...
        {
            throw "Additive scale fix Revision Export failed for $Asset with exit code $LASTEXITCODE"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Additive scale fix smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    $PolicyValue = [ordered]@{
        ...
        allowedOperations = @("setAnimationScaleFix")
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Additive Scale Fix smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Additive Scale Fix smoke test did not complete successfully."
}
Write-Host "MCP Additive Scale Fix smoke test passed."
