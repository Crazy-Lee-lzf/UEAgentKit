...
$ClientScript = Join-Path $ToolRoot "tests\integration\mcp_live_diagnose_additive.py"
...
$Output = Join-Path $ToolRoot "Output\McpAdditiveDiagnoseSmoke"
...
$BackupRoot = Join-Path $ToolRoot "Backups\McpAdditiveDiagnoseSmoke"
...
try
{
        ...
        "/Game/Characters/XinYueHu/Mesh/SK_XinYueHu"))
    {
        ...
        {
            throw "Additive diagnosis Revision Export failed for $Asset with exit code $LASTEXITCODE"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "Additive diagnosis smoke index build failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "MCP Additive Diagnosis smoke test failed with exit code $LASTEXITCODE"
    }
    $Succeeded = $true
}
finally
...
if (!$Succeeded)
{
    throw "MCP Additive Diagnosis smoke test did not complete successfully."
}
Write-Host "MCP Additive Diagnosis smoke test passed."
