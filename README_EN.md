...
> **Development status**: 0.6.0 remains the latest published release. In the parallel 0.7.0-dev tracks, `feature/memory-context` now implements the local single-user Schema v3 MVP: a Knowledge Tree, Active Work, levels 0–4 progressive Context, on-demand Evidence, and five high-level MCP tools; `feature/live-editor-realtime-io` continues expanding real-time reads and writes in the running Unreal Editor. Without Memory the modes expose 5/23/29/47 tools, and fixed Project Memory changes them to 17/35/41/59. Automatic Context Packs, analysis, and a shared multi-user knowledge service remain future work.
