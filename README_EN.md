...
The MCP Client still uses local `stdio` only. Default mode exposes 5 offline read-only tools; `-EnableLiveEditor -ProjectPath <fixed project>` adds 10 live read-only tools for a total of 15; the fixed-project workflow exposes 18; combining both exposes 28. Live mode includes bounded Output Log queries, compile diagnostics, non-loading live asset inspection, and focused Graph/Node selection for ordinary Blueprint Editors; workflow mode adds four-source asset state and safe single-asset index refresh:
