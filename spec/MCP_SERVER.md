...
两个破坏性 Tool 显式回退最近一次已确认的 Live Apply，不需要 Plan：Undo 通过 `GEditor->UndoTransaction(true)` 撤销（可 Redo），Discard 通过 `GEditor->UndoTransaction(false)` 彻底丢弃；两者都只作用于 `ue_apply_asset_property_live` 返回的精确事务：
...
执行前重新校验 Session 身份、资产内存对象/Class/Package 身份、包未被保存以及 Editor Undo 栈顶就是该事务（`GetUndoContext` 的 TransactionId 与 PrimaryObject 匹配）；期间发生其他编辑、包被保存或 Editor 重启都会拒绝并要求重新 Plan。回退后显式恢复写入前 Dirty 状态，并用写入前 Snapshot 读回验证目标值，成功后该事务的 Undo 记录删除（二次回退返回 not-found）。返回 `mode`（`LiveUndo`/`LiveDiscard`）、`operation`、`valueKind`、`assetPath`、`transactionId`、`editorSessionId`、回退前后值、`dirtyBefore`/`dirtyAfter` 与 `saved=false`/`diskRevisionChanged=false`；整个过程不触碰磁盘、SQLite 或 Revision Export。

### `ue_verify_live_write`（Apply → Save → Verify → Memory 闭环）

该规划类 Tool 把一次 Live Apply 收尾成标准闭环：`ue_apply_asset_property_live`（记录 `liveApplyReceipt`）→ 用户显式 `ue_save_authorized_asset` Preview/Commit（Commit 成功后该 Live Apply 记录被标记 `saved`）→ `ue_verify_live_write(asset_path)`：

- 若目标包仍 Dirty：返回 `state=not-saved` 终态（`saved=false`、`verified=false`、`undoAvailable=true`、磁盘 Revision 未变），不伪装成功；`memoryTaskEvidence` 以 `cancelled` 结论记录"未持久化，可保存或撤销"，下一步提示继续授权保存或 Undo/Discard（覆盖"Live Apply 后不保存直接退出"）。
- 若包干净但未经过授权保存（外部保存/手工撤销/Session 分叉）：拒绝 `live-write-verify-save-unauthorized` 并要求重新 Plan。
- 若已授权保存：独立 Unreal 进程重载磁盘资产（RunAssetCatalog），校验导出的资产路径、干净的 SHA-256 Revision 与磁盘一致、Revision 相对冻结索引已变化（`live-write-verify-revision-unchanged` 拒绝未持久化），并按 Kind 提取导出值（DataAsset 属性 / MI 参数 / DataTable 行字段）与应用时 `afterValue` 做 JSON 级比对（`live-write-verify-value-mismatch` 拒绝保存前后被人工改动）。通过后返回 `state=verified`、`actualRevision`、`expectedValue`/`exportedValue`、`undoAvailable=false`，并生成 `memoryTaskEvidence`（`succeeded` 结论，task_key=`live-write:<planId>`，含 Plan/Apply（transactionId、undo 可用性）/Save（授权保存 backup-manifest 引用）/Verify（独立重载报告）证据与最终 Revision Set）。
