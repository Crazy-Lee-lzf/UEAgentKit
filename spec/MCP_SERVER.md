...
- 输出受 `max_output_tokens` 强制约束：超预算时按固定优先级阶梯裁剪可展开内容（Change Set operations → Live Editor summary → Memory records/nodes → Active Work items → Relevant Assets 候选 metadata → 候选数量 → 目标资产 metadata/summary → Revision comparisons → project stats → nextExpansions → risk details → 深层标识字段），并在 `outputBudget.truncated/truncationReason` 中显式报告；裁剪的展开路径进入 `nextExpansions`。候选永不优先于 target identity、high risk 与 revision summary 等更高优先级信息。
