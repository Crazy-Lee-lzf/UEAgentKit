...
UE Agent Kit 0.8.0 treats the SQLite index and Revision Export as fixed read snapshots. A saved Unreal package can change after those snapshots were built, so the MCP server must never assume that an available index is automatically current.
